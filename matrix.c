...
static
void __multiplyMV(float* r, const float* lhs, const float* rhs)
{
...
void floatcopy(float *src, int srcOffset, float *dest, int destOffset, int len)
{
	...
	{
		__multiplyMM(result+resultOffset, lhs+lhsOffset, rhs+rhsOffset);
	}	
	...
	void multiplyMV(float *result, int resultOffset,
            float *lhs, int lhsOffset, float *rhs, int rhsOffset)
	{
		__multiplyMV(result+resultOffset, lhs+lhsOffset, rhs+rhsOffset);
	}			
     ...
     */
	static int invertM(float *mInv, int mInvOffset, float *m,
            int mOffset) {
    ...
    void rotateM(float *m, int mOffset,
            float a, float x, float y, float z) {
        {
            ...
            floatcopy(sTemp, 16, m, mOffset, 16);
        }
    }
